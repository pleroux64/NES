...
{
    uint8_t value = memory[addr]; // Read the value at the address
    value++;                      // Increment the value
    memory[addr] = value;         // Write the new value back to memory
...
{
    uint8_t value = memory[addr]; // Read the value from memory
    std::cout << "Initial value: " << std::hex << (int)value << std::dec << std::endl;

    value--;              // Decrement the value
    memory[addr] = value; // Write the decremented value back to memory
...
#pragma region ADC Opcodes
    ...
    opcodeTable[0x75] = [](CPU &cpu) {                     // ADC Zero Page, X
        ...
        uint8_t value = cpu.memory[(addr + cpu.X) & 0xFF]; // Get the value at addr + X (zero page wrapping)
    ...
    opcodeTable[0x7D] = [](CPU &cpu) {                       // ADC Absolute,X
        uint16_t addr = cpu.fetchWord();                     // Fetch the 16-bit address
        uint8_t value = cpu.memory[(addr + cpu.X) & 0xFFFF]; // Fetch the value at addr + X (wrap if needed)
    ...
    opcodeTable[0x79] = [](CPU &cpu) {                       // ADC Absolute,Y
        uint16_t addr = cpu.fetchWord();                     // Fetch the 16-bit address
        uint8_t value = cpu.memory[(addr + cpu.Y) & 0xFFFF]; // Fetch the value at addr + Y (wrap if needed)
    ...
    opcodeTable[0x61] = [](CPU &cpu) {                                                                       // ADC (Indirect,X)
        ...
        uint16_t indAddr = cpu.memory[(addr + cpu.X) & 0xFF] | (cpu.memory[(addr + cpu.X + 1) & 0xFF] << 8); // Fetch the 16-bit address
        uint8_t value = cpu.memory[indAddr];                                                                 // Fetch the value at the indirect address
    ...
    opcodeTable[0x71] = [](CPU &cpu) {                                     // ADC (Indirect),Y
        uint8_t addr = cpu.fetchByte();                                    // Fetch the base address (zero page)
        uint16_t indAddr = cpu.memory[addr] | (cpu.memory[addr + 1] << 8); // Fetch the 16-bit address
        uint8_t value = cpu.memory[(indAddr + cpu.Y) & 0xFFFF];            // Fetch the value at (indAddr + Y)
