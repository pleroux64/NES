...
#pragma region JMP Opcodes
    ...
    opcodeTable[0x6C] = [](CPU &cpu)
    {
        ...
        uint16_t lowByte = cpu.readMemory(pointer); // Fetch low byte
        uint16_t highByte;

        // Handle the CPU bug for page crossing
        if ((pointer & 0x00FF) == 0x00FF)
        {
            highByte = cpu.readMemory(pointer & 0xFF00); // Wrap around the page
        }
        else
        {
            highByte = cpu.readMemory(pointer + 1); // Fetch high byte
        }
